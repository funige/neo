...
オリジナルのPaintBBSのv2.04やv2.19では`oekaki.html`の記述通りに動作します。   
そのため、いにしえのオリジナルのPaintBBS v2.04と同じように右クリックでコピーをリセットできるようにしました。  

#### ver1.6.31 (2026/04/11)
- 右クリックによるベジェのリセット機能を追加しました。
オリジナルのPaintBBSには右クリックによるベジェのリセットができないバージョンがありますが、v1.46 v2.04 v2.19といったバージョンでは右クリックによるベジェのリセットが可能でした。
しぃペインターも同様です。    
そのため、右クリックでベジェをリセットできるようにしました。
- NEO v1.5.xの頃から存在していた、コピー操作キャンセル時に、コピー移動中画面の不透明度100%の画像が残る問題を修正しました。
- コピーする選択範囲の確定時に、選択範囲内の画像がコピー移動時の不透明度100%の画像に切り替わる問題を修正しました。
