...
お絵かきしぃ掲示板 PaintBBS (&copy;2000-2004 しぃちゃん) を html5 化するプロジェクトです。

作者行方不明のため、許諾は取れていません。

[しぃちゃんのホームページ（Vector)](https://web.archive.org/web/20070206095246/http://hp.vector.co.jp/authors/VA016309/)  

**Flash や html5 のお絵かきサイトはいっぱいあるんだけど、そうじゃないんだ。  
...
### Petit Note
- [お絵かき掲示板 Petit Note](https://paintbbs.sakura.ne.jp/)
   
[POTI改 設置サポート掲示板](https://paintbbs.sakura.ne.jp/cgi/neosample/support/)で、POTI-board EVOと、Petit Noteのサポートを行っています。
...
```
### Chrome(80 以降？)で横に長い線を引いた時にジェスチャーと認識されて、ブラウザバックが発生する場合
- chrome://flags/#overscroll-history-navigation でジェスチャーを無効にするのもひとつの方法です。
- PCの場合は以下のCSSで解決する事があります。
```
<style>body{overscroll-behavior-x: none !important; }</style>
```
ただし、SafariやAndroidのChromeではこのCSSを使った場合でもブラウザバックが発生するようです。  
  
### 外部ライブラリのカラーピッカーと連携するためのインターフェース
- これまでは、外部ライブラリのカラーピッカーと連携させるには、PaintBBS NEOそのものを改造する必要がありました。    
そのため、外部ライブラリのカラーピッカーにNEO側で変更した色を渡すためのインターフェースを作成しました。  
```
...
## <a name="history">履歴</a>
#### ver1.7.32 (2026/09/25)
- `neo_viewer_max_width_100`がtrueに設定されている時は、動画再生ビューワーの横幅を`max-width:100%`に変更します。  
これにより、スマホのような幅の狭い端末で動画を再生した時にも横スクロールが発生しなくなり、ほとんどのケースで画面全体を表示しながら動画を再生する事ができるようになります。    
横幅の変化にあわせてビューワーの高さや再生位置のマークも変化します。  
#### ver1.7.31 (2026/09/20)
- 1.5倍表示、2.5倍表示の時に1px<->2pxの中間を表現できずどちらかに偏っていた問題を修正しました。  
v1.7.29でズームアウトのオプションが有効な時には1.5倍表示と2.5倍表示もできるようにしましたが、非常に汚い描線になっていました。  
1pxの筈の線が場所によっては1px、あるいは2pxで表示されていました。  
スムージング処理を行えばその問題は解消しますが、画面全体がぼやけるため、NEOのようなジャギーが見えなければならないペイントツールには不向きでした。    
そのため、1.5倍の時には「1.5倍の2倍｣=3倍に拡大してから1/2に縮小し1.5倍にするようにしました。      
2.5倍の時には「2.5倍の2倍=5倍に拡大してから1/2に縮小し2.5倍にします。    
この処理は通常よりもメモリを多く消費するため、表示されている領域にのみこの処理を適用して過大なメモリ消費量にならないようにしました。  

#### ver1.7.30 (2026/09/17)
- テキスト入力で`[]`を入力した時に、文字サイズ(ブラシサイズ)が変化するバグを修正しました。   
このバグはv1.7.28で発生し、v1.7.30で修正されました。  
#### ver1.7.29 (2026/09/16)
- ズームアウトを許可するパラメータ`Neo.config.neo_enable_zoom_out`が`true`に設定されている時の表示倍率に1.5倍と2.5倍を追加しました。     
窓(全画面モード)で2倍に拡大した時に画面のスクロールが必要になり画面全体を表示できない場合でも1.5倍であれば画面全体を表示できる場合があるからです。  
#### ver1.7.28 (2026/09/15)
#### `]`でブラシサイズを1px上げる`[`でブラシサイズを1px下げる
- ブラシサイズを変更するショートカットキーを追加しました。  
- `]`キーでブラシサイズが1px上がり`[`キーでブラシサイズが1px下がります。      
以前から、CTLR+ALT+ドラッグでブラシサイズを変更するショートカットキーはありましたが、新しく追加されたショートカットキー`[` `]`を使用すると1px刻みでブラシサイズを変更する事ができます。    
(一般的なペイントツールのブラシサイズ変更のショートカットキーと同じ動作)
#### ver1.7.27 (2026/09/14)
- スマホ･タブレットなどのタッチイベントが有効なデバイスでキャンバスの上端から線を引くことができなくなっていたのを修正しました。   
NEO独自パラメータ`neo_disable_grid_touch_move`がtrueに設定されていて、さらに画面の横幅に余裕がある時にはこの問題は発生していませんでしたが、横幅が狭い端末の時にはキャンバスの上端から線を引くことができなくなっていまいた。
#### ver1.7.26 (2026/09/09)
- NEO独自パラメータ`neo_disable_grid_touch_move`で、touchmoveイベントのデフォルトの動作をキャンセルする条件に｢窓｣(全画面モード)を追加しました。        
全画面モードの時には画面がフィットしているため上下に動く事はありませんが、ジェスチャーによるリロードは発生していました。      
このジェスチャーは｢四角｣｢レイヤー結合｣｢傾け｣で発生するため、これらの操作を行った時にブラウザがリロードしようとしていました。  
そのため、全画面モードの時にもtouchmoveイベントのデフォルトの動作をキャンセルしてリロードを防止するようにしました。      
`neo_disable_grid_touch_move`を設定していない時は、全画面モードでもスクリーンの幅が広い端末でもリロードなどのジェスチャーが発生しますのでご注意ください。      
この設定がfalseにも設定できる理由は、外部のjsで別途制御しているケースがあるからです。      
#### ver1.7.25 (2026/09/08)
- v1.7.19でキャンバス全体を選択した時に｢傾け｣が1pxずれる問題を修正した時に、選択範囲のサイズが小さな時に｢傾け｣が1pxずれるようになっていたのを修正しました。  
#### ver1.7.21 (2026/09/08)
- ｢傾け｣の1pxのずれを修正した時に時に発生したバグにより｢四角｣｢消し四角｣などが1pxずれるようになっていたのを修正しました。   
このバグはv1.7.19で発生し、v1.7.21で修正されました。  
#### ver1.7.20 (2026/09/07)
- キャンバスをズーム機能で拡大した時に、スクロール可能領域のずれ生じていたのを修正しました。   
このスクロール可能領域のずれが原因となり、キャンバスの端で円カーソルのグリッチが発生していたのを修正しました。 
#### ver1.7.19 (2026/09/07)
- ｢傾け｣使用時に1pxずれる問題を修正しました。
...
- ブラシサイズが8px以上の時の補正強度がより弱くなるようにしました。
- ズームで拡大している時の補正強度がより弱くなるようにしました。
#### JSDocの追加と修正
- 今後の開発のために、any型にせざるを得ないと判断していた箇所の型を明確にしました。
#### ver1.7.15 (2026/07/05)
#### テキスト入力のバグを修正
...
#### ver1.7.12 (2026/07/04)
#### 手ぶれ補正levelの初期値の見直し
- NEO本体の手ぶれ補正の初期値を1から0に変更しました。  
これにより、外部で`Neo.setStabilizeLevel(1)`のように値をセットした時にのみ手ぶれ補正機能が有効になるようになりました。  
そのため、手ぶれ補正を無効化するフラグも必要なくなりました。  
#### ver1.7.11 (2026/07/02)
#### class構文に全面的に移行
...
- sampleのカラーサークルとNEOの色の設定にループが発生していたのを修正しました。  
ループ防止のフラグを追加しました。    

#### ver1.7.10 (2026/06/28)
- ウィンドウビュー(全画面)モードに切り替わった事を通知するカスタムイベントを追加しました。  
詳細はこのページ内の上部をご確認ください。  
また、sampleにもイベントリスナーのコードを追加しました。   
ウィンドウビューモードの時には`true`を、ページビューの時には`false`をconsoleに出力します。  
#### ver1.7.9 (2026/06/27)
- 外部ライブラリのカラーピッカーと連携するためのインターフェースを作成しました。
sampleとsample2に[luncheon/reinvented-color-wheel: A vanilla-js touch-friendly HSV color picker inspired by Farbtastic Color Picker.](https://github.com/luncheon/reinvented-color-wheel)を実装しましたので、そちらを確認していただいたほうが早いかもしれません。  
#### ver1.7.8 (2026/06/25)
...
しかしながら、もっとも確実な回避方法は、Chromeの設定の ｢常にアクティブにするサイト｣ にサイトを追加する事です。    
**デフォルトはfalse** です。 
...
#### ver1.7.2 (2026/05/31)
これまでは新しく開発した掲示板であっても、Javaアプレット形式の設定が必須でした。    
しかしながら`param`タグは2022年にHTML Living Standardから削除されており、突然使用できなくなる可能性があります。      
そのため、データーセットと、オブジェクトを使用した起動ができるようにしました。  
しかし、互換性は維持する必要があります。  
従来のapplet-dummy、paramタグでも問題なく動作するようにしました。  
sampleは、applet-dummyとparamで動作し、sample2は新しいデーターセットとオブジェクトの設定を読み込んで動作します。  

[具体的な設定例](./README-params.md#applet-dummy%E3%82%BF%E3%82%B0%E3%81%A8param%E3%82%BF%E3%82%B0%E3%82%92%E4%BD%BF%E7%94%A8%E3%81%97%E3%81%AA%E3%81%84%E6%96%B0%E3%81%97%E3%81%84%E8%A8%AD%E5%AE%9A) 。

新しい設定方法のほうがより少ない文字数で設定できます。  

#### ver1.6.61 (2026/05/15)
- Object.create(Parent)をclass構文のextendsを使用した継承に書き直しました。  
new(Parent)が残っていた箇所もclass構文のextendsを使用した継承になりました。
#### ver1.6.60 (2026/05/10)
- new(Parent)で継承するのではなくObject.create(Parent)で継承するようにしました。
- 1000件以上あったTS型チェックの警告を10件に削減しました。  
そのうち8件の該当コードはライブラリなので、実質2件です。
JSにTSの型チェックを厳格に行うと膨大な数の警告になるため、暗黙的にanyなどの警告はチェック対象から除外しました。
- v1.6.43 (2026/04/23)で、ツールメニューに追加した`touch-action: none;`が効果的に動作している事が判明しました。
それ以前のバージョンでは、Touchイベントが割り込んでスムーズにスライダーが動いていませんでした。
  
#### ver1.6.59 (2026/05/08)
- `Neo.setStabilizeLevel()`でも`Neo.setStabilizLevel()`でも手ぶれ補正の強度を指定できるようになりました。  
StabilizLevelがスペルミスでeが足りないため外部から呼び出す関数名も変更になりましたが、既存のシステムでも動作するようにする必要があるため、どちらでも呼び出し可能になるようにしました。    
- TS型チェックの静的解析ツールを使ってコードの整理をすすめています。   
Javascriptのチェックで使うには厳格すぎるので、設定ファイルを追加してチェックを若干緩くしてチェックしています。  

#### ver1.6.58 (2026/05/06)
- ショートカットキーを追加しました。
- E:消しゴム、B:鉛筆、W:水彩、Delete|BackSpace:全消し(全消しツールへの切り替えではなく全消しを実行します)
#### ver1.6.57 (2026/05/05)
- ｢お絵描きしぃ掲示板 PaintBBS」→｢お絵かきしぃ掲示板 PaintBBS｣(正式名称)に修正。  
- コード整理。  
#### ver1.6.56 (2026/05/04)
...
#### ver1.6.53 (2026/05/01)
- ショートカットキーでブラシサイズを変更するとスライダーが操作ができなくなるv1.6.52のバグを修正しました。
#### ver1.6.52 (2026/04/30)
- コード整理。v1.6.51で修正したver1.6.46で線幅が保存できなくなる問題の原因を解明して、コード内のコメントに記載しました。
#### ver1.6.51 (2026/04/29)
#### バグ修正
- ver1.6.29でidに接頭辞`neo-`を追加した時に、
`this.params.type == "neo-pen"`
に変更していなかったため、鉛筆が選択されていない状態で起動していたのを修正しました。    
- 鉛筆･水彩などのツールの切り替えのたびに線幅が1にリセットされるバグを修正しました。  
このバグはver1.6.46のtouchstart、touchendのpointerdown、pointerupへの置き換えにより発生し、v1.6.51で修正されました。  
- 以前から存在していたテキストツールで文字入力画面を開いて確定していない状態でツールを切り替えた時に、テキストツールの入力欄が消えずに残る問題を修正しました。  

#### バグストライプをなくすオプション
- オリジナルのPaintBBSに存在しいて、NEOでも再現する｢傾け｣のバグを使ったストライプ模様の作成を利用したい場合もあれば、傾けたいだけなのにストライプ模様が出現して、それを消す作業が必要になる場合もあります。    
そのため、バグストライプが発生しないようにするオプションを追加します。  
  
- **&lt;PARAM NAME="neo_disable_turn_original_glitch" VALUE="true">**  
このフラグをtrueにすると、バグストライプが発生しなくなります。  
[ぴくらぼ](http://piclab.sakura.ne.jp/kouza/straip/straip.htm)  
バグストライプ使用例。  
オリジナルのPaintBBSではバグストライプが発生するため  
**デフォルトはfalse** です。  

#### ver1.6.48 (2026/04/27)
- iPadやiPhoneで、意図せずダブルタップズームが動作して画面が拡大する問題に対応するため、ダブルタップのデフォルトの動作をキャンセルする処理を追加しました。
#### ver1.6.47 (2026/04/25)
- `<PARAM NAME="neo_disable_grid_touch_move" VALUE="true">`でアプレットのグリッド部分のタッチスクロールを無効化した時に発生していた問題を修正しました。
アプレットの左右の余白部分でタッチスクロールできなくなっていたのを修正しました。
- `pointerdown`に切り換えていたにも関わらず`touchstart`を判定する処理が残っていたを修正しました。    

#### ver1.6.46 (2026/04/24)
- 描画とスライダー操作のEventListenerを、pointerEventsに統一しました。
- 手ぶれ補正onの時の補正強度を弱めました。  
レベル1の時の補正レベルを`0.55`に引き下げました。(数字が小さいほど補正があまりかからない)  

#### ver1.6.43 (2026/04/23)
- キーボードイベント関連の軽微なエラーを修正しました。

#### ver1.6.42 (2026/04/22)
- 手ぶれ補正onの時の補正強度を弱めました。  
v1.6.41でレベル1相当だった補正レベルはv1.6.42ではレベル2になりました。  
ほぼ1レベル分ずつシフトしています。  
レベル1は、以前は`0.8`でしたが、今は`0.6`です。(数字が小さいほど補正があまりかからない)  
#### ver1.6.41 (2026/04/20)
- スペースキー押下で手のひらツールに切り替わらない事があるバグを修正しました。
- キーダウンのたびにEventListenerが追加されていたバグを修正しました。

#### ver1.6.39 (2026/04/19)
- 手ぶれ補正のレベルを指定できるようになりました。
`Neo.setStabilizeLevel()`の引数に`0`から`5`までの数字をセットしてレベルを指定します。
0で補正なし、5で最強。
`sample/`のテストページにも、手ぶれ補正のレベルを設定する項目を追加しました。
補正レベルの切り替えを簡単にテストする事ができます。    
#### ver1.6.38 (2026/04/18)
- コードを整理しました。
#### ver1.6.37 (2026/04/18)
- ver1.6.36のバグを修正しました。
手ぶれ補正機能を追加したあとの動作確認で、ペンやマウスは問題なかったのですが、スマホ+指では描画できなくなっていました。
ポインターアップ時にも座標の更新を行うようにして、スマホ+指でも補正のかかった状態で描画できるようにしました。
#### ver1.6.36 (2026/04/17)
- 手ぶれ補正機能を追加しました。  
キャンバスを60%や40%、20%などに縮小表示しながら描画した時に高い効果を発揮します。    
しかしながら、手ぶれ補正を使いたくない場合もありえます。  
そのため、以下のパラメータを追加しました。  
- ~~**&lt;PARAM NAME="neo_disable_stabilizer" VALUE="true">**~~  
~~このフラグをtrueにすると、v1.6.36で追加した手ぶれ補正機能を無効化します。~~  
v1.7.12で、手ぶれ補正の値がセットされていない時には、手ぶれ補正が無効になったため、このフラグは廃止になりました。 
#### ver1.6.35 (2026/04/16)
...
#### ver1.6.34 (2026/04/15)
- コードを整理しました。
#### ver1.6.33 (2026/04/14)
- コピー操作中に｢ctrl+z｣や｢元に戻す｣ボタンでアンドゥした時の動作を変更しました。  
｢Esc(エスケープ)｣や｢右クリック｣などによる操作のキャンセルと同じ動作になりました。   
これまではアンドゥで枠線が消えても、ペーストモードが解除されずに残っていました。      
そのため、ドラッグで画像が移動しペーストされていました。    

#### ver1.6.32 (2026/04/12)
...
そして、オリジナルのPaintBBSのv2.04やv2.19は`oekaki.html`の記述通りに動作します。   
そのため、右クリックでコピーをリセットできるようにしました。  
...
#### ver1.6.31 (2026/04/11)
- 右クリックによるベジェのリセット機能を追加しました。
オリジナルのPaintBBSのv1.46 v2.04 v2.19では右クリックによるベジェのリセットが可能でした。   
しぃペインターも同様です。    
そのため、右クリックでベジェをリセットできるようにしました。

#### ver1.6.30 (2026/04/08)
- ペンを離した際に一瞬表示される、ベジェプレビューの表示のぶれを修正しました。
#### ver1.6.29 (2026/04/07)
- `Viewer`を含むすべてのidに接頭辞"neo-"を追加しました。
#### ver1.6.28 (2026/04/06)
- 掲示板やCMSのidと重複が発生する可能性があるためより多くのidに接頭辞"neo-"を追加しました。  
動画再生画面の`Viewer`からはじまるidには接頭辞を追加できませんでした。   
その他のidには接頭辞"neo-"が追加されています。  
#### ver1.6.27 (2026/04/05)
- 掲示板やCMSの、id `#container`とNEOのid `#container`の重複が原因で、起動に失敗するケースがあるため、すでに使用されている可能性の高いidに接頭辞"neo-"を追加しました。
#### ver1.6.26 (2026/03/23)
- requestAnimationFrame()を使用した影響で、四角ツールのXOR反転表示がドラッグ終了後も残るバグを修正しました。
- **&lt;PARAM NAME="neo_enable_zoom_out" VALUE="true">** でキャンバスの縮小表示を許可した時の縮小表示倍が80%、60%、40%、20%になりました。
#### ver1.6.25 (2026/03/15)
- レイヤー結合やコピーの操作を行った時の全体更新の頻度をrequestAnimationFrame()で最適化して、負荷を削減しました。
#### ver1.6.24 (2026/03/13)
- 0.5倍の縮小表示の表示品質が必要以上に高く設定されていました。  
そのため、2000x2000Pixelのような大きなキャンバスを開いた時にスクロールバーの操作ができなくなっていました。  
`ctx.imageSmoothingQuality = "high";`    
を0.5倍未満の時にのみ使用するようにして、0.5倍表示時の負荷を削減し、スクロールバーが操作できるようにしました。
- スクロールするたびにキャンバスの全体更新が実行され処理が重くなるため、requestAnimationFrame()を使用して全体更新の頻度を下げました。  

#### ver1.6.23 (2026/01/20)  
- ブラシサイズが10px以下の時にのみ、描線がカクカクする問題を解消していましたが、16px以下に適用範囲を拡大しました。  
- ペンが浮いた状態の時のカーソルの移動で高精度なイベントを処理しても負荷が高くなるだけで意味がないため、描画時のにみ高精度なイベントを処理するようになりました。  

#### ver1.6.22 (2026/01/17)
- ペンを素早く動かした時に、描線の円弧が正しく描写されずカクカクした線になる問題を修正しました。  
ただし、ブラシサイズが10px以下の時のみの対応になります。    
描線のカクツキを解消するために従来よりも多くのイベントを処理しており、ブラシサイズが大きな時は負荷が問題になるからです。
...
極めて大きなサイズの動画の再生時や、小さな画面のスマホ等のデバイスで動画を縮小表示して再生するためのUIです。  
...
#### ver1.6.19 (2025/10/11)
- `2chan.net`ではない別サイト（例 `example2chan.net`など）の時にふたば専用モードになる問題を修正しました。
