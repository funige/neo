...
## <a name="history">履歴</a>
#### ver1.7.5 (2026/06/12)
- TS型解析ツールをJSに適用して、適切な型を指定しました。  
暗黙的にanyになっていた箇所に型を指定するとnullかもしれないという警告や、整合性がとれないという警告が多数発生しますがひとつひとつ修正していきました。  
また暗黙的にanyになるという警告も解消しました。   
コードの内容からanyにせざるを得ない箇所を明示的にanyにし、型を明確にできる箇所には型を追加しました。  
この作業には約一ヶ月かかりましたが、この作業によりコード内の関数をマウスオーバーするだけで関数のパラメータや返り値の型がわかるようになりました。  
また、コードに矛盾があれば新たな警告が発生するため、バグを未然に防ぐ事ができます。 
