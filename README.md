...
- **&lt;PARAM NAME="neo_disable_grid_touch_move" VALUE="true">**  
  ...
  **デフォルトは false です**  
- スポイトのショートカットキー｢Alt｣を押下した時にFirefoxのメニューが開閉するのを防止する処理を追加しました。
- ver1.6.11で追加したキャバスへの動画データ読み込み時に、アニメーション再生をスキップする機能を使った時には、静止画を取得以 前のアンドゥの履歴を動画から取得しないようにしました。  
  これまでは、静止画を取得した直後にアンドゥすると画面が空白になっていました。
