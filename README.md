...
- NEO独自パラメータ`neo_disable_grid_touch_move`をtrueに設定すると画面の幅に余裕がある時にtouchmoveイベントのデフォルトの動作をキャンセルして、画面が上下に動かなくする処理はすでにありました。  
そのtouchmoveイベントのデフォルトの動作をキャンセルする条件に｢窓｣(全画面モード)を追加しました。   
全画面モードでは画面が上下に動く事はありませんが、上から下に向かって指をスライドした時にリロードが発生していたため、ジェスチャーによるリロード防止のため窓モードを条件に追加しました。   
この上から下への指のスライドは、｢四角｣｢レイヤー結合｣｢傾け｣で発生するため、これらの操作を行うとリロードがはじまっていました。   
`neo_disable_grid_touch_move`を設定していない時は、全画面モードでもスクリーンの幅が広い端末でもリロードなどのジェスチャーが発生してしまいますので、ご注意ください。   
