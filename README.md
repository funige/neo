...
`<select onchange="Neo.setStabilizLevel(this.value)">`といったHTML+Javascriptで、`Neo.setStabilizLevel()`の引数に0から5までの数字をセットしてレベルを指定します。
0で補正なし、5で最強。
`sample/`のテストページにも、手ぶれ補正のレベルを設定する項目を追加しました。補正レベルの切り替えを簡単にテストできます。  
