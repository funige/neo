...
#### ver1.7.5 (2026/06/04)
#### バグ修正
- v1.5の時点から存在していたモバイル端末でBzの操作を完了しないまま、手書きや直線への切り換え時にBzのハンドルが表示されたままになるバグを修正しました。
#### TS型解析ツールの警告を削減
- ver1.6.60で｢1000件以上あったTS型チェックの警告を10件に削減｣したつもりになっていましたが、多くの型がanyになる事で傾向がでなくなっていただけの箇所が数多くありました。
型を明確にした結果、800件の警告が発生したため、JSDocの追加や修正、またコードそのものの書き直しを行いました。
ライブラリとして仕様しているlz-stringの警告は残っていますが、ライブラリの警告はあえて残しました。
でなければライブラリがバージョンアップするたびにライブラリの警告を解消するための更新作業が発生するからです。
#### Neo.params
